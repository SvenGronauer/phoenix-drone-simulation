...
def optimize_with_Adam(args):
    ...
    # step size used for numerical approximation of the Jacobian via
    # forward differences:
    epsilon_vector = np.array([0.005, 0.005, 0.005])
    mpi.mpi_print('epsilon_vector:', epsilon_vector)

    def estimate_gradient(_x: np.ndarray,
                          mu: Tuple[float, np.ndarray] = epsilon_vector,
                          N: int = 128,
                          shrink: int = 128
                          ) -> tuple:
        r"""Calculates Jacobian based on finite-difference approximation."""
        rets = []
        derivative = np.empty((N, _x.size))
        for i in range(N):
            u = np.random.normal(size=_x.size)
            w_1 = _x + mu * u
            w_2 = _x - mu * u
            a = obj_func.evaluate(params=w_1, shrink=shrink)
            b = obj_func.evaluate(params=w_2, shrink=shrink)
            der = u * (a - b) / (2. * mu)
            rets.append((a + b) / 2.)
            derivative[i] = der
        gradient = np.mean(derivative, axis=0)
        assert gradient.size == _x.size
        return gradient, np.mean(rets)
    ...
    for epoch in range(500):
        ...
        grad, F = estimate_gradient(params.detach().numpy())
